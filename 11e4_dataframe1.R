...
table(grades)
(marks1 = trunc(rnorm(n=sizeN, mean=75, sd=5 ))) # needs meand and sd
(marks2 = trunc(runif(n=sizeN, min=60, max=95 ))) # needs min and max
...
str(df1)  #structure of data, tells you waht datatype each column has ,and gives sample from each one
attributes(df1)
...
#selected 
df1[1:4, 1:5]# filtering first 4 rows and first 5 columns
df1[1:5,1:3]
hist(df1$marks1)
df1[1:5, c('rollno', 'name', 'gender')]
df1[df1$gender == 'M', c('rollno', 'name','gender')]
df1[df1$gender == 'M' & df1$marks1 > 80, c('rollno', 'name','gender')]
df1[df1$grades %in% c('A','C') & df1$marks2 > 80 & df1$marks1 < 75, c('rollno', 'name','gender', 'grades', 'marks1', 'marks2')]# if you have one column than you can use dollar symbol
df1[df1$grades %in% c('A','C') & df1$marks2 > 80 | df1$marks1 < 75, c('rollno', 'name','gender', 'grades', 'marks1', 'marks2')]# pipe ymbol is used as "OR" things
#summary
aggregate(df1$marks1, by= list(df1$gender), FUN=max)# maximum marks scored by each gender in marks 1
aggregate(cbind(marks1, marks2) ~ gender + course , data=df1, FUN=mean)# mean marks of each course and gender
...
#factors
(genderF = factor(gender))  #nominal data(without an order)
summary(genderF)
summary(gender)
(courseF = factor(course)) #course data
summary(courseF)
(gradesOF = factor(grades, ordered=T, levels = c('C','B','A','D')))#D is highest, ordered factor, while creating ordinal data
summary(gradesOF)
