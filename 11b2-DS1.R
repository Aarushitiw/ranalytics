...
x[22]
x[22:25]
x[seq(1,50,2)]# every 2 nd value
x
x[seq(1,50,5)]#every 5th value
...
#access elements
?seq
(x6 = seq(0,100,by=3))#creating a vector by name of x from sequence 0 to 100 with interval of 3
seq(0,100,3)
seq(to=100,from=0,by=3)
seq(1,5,2)
?seq
#[1]  0  2  4  6  8 10
ls()  #variables in my environment- list the variables in the environment
x6
length(x6)
?length
x6[1]; x6[21]
...
x6[-c(1:10, 15:20)]#removed the elements from 1 to 10 and 15 to 20
...
x6[-c(1,5,20)]#removing 1st 5th and 20th
x6
x6[x6 > 20]

x6[x6 > 30 & x6 < 40]  # 31-39# marks of students between 30 & 40
x6[x6 != 30]
#or | and is &  !
length(x6)
x6
x6[-(length(x6)-1)]# 33rd value removed
x2
(x7 = c(x6, x2))
...
#modify
x6
set.seed(1234)# same values to come and when you want to replicate data , u will get the same sequence
(x6 = sample(1:50))
...
sort(x6[-c(1,2)])
sort(x6, decreasing=T)# if F then it will cause ascending
x6
rev(x6)# x 6 sequence has been reversed
...
(x = -3:2)
x6
x6[2:10] <- 99; x6        # modify 2nd element , value from 2nd to 10h element have been replaced by 99
x6[x6 > 30 & x6 < 40] = 999
x6
...
x
diff(x)# difference between the above line consecutive components
x = NULL
x
#NULL
x[4]
...
?rnorm
(x = rnorm(100))
plot(density(x))# gives the graph thatis required
abline(v=c(-3,0,3))# +- 3 sigma values have been put up
mean(x)
(x1 = rnorm(100, mean=50, sd=5))# trying o create nirmally distributed data with mean as 50
mean(x1)
plot(density(x1))
...
hist(x1)
hist(x1, freq=F)
lines(density(x1), col=2)# density curve has been plotted on mid point of each bar
summary(x1)
quantile(x1)
