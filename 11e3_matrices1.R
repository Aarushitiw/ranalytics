...
m1
dimnames(m1)= list(c('Jan','Feb','Mar','Apr'), paste('coy',1:6, sep=''))# separator usage , paste concatenates 
m1 #4 months, 6 coys
m3
...
m3
t(m3)  #transpose
sweep(m3, MARGIN=1, STATS=c(2,3,4,5,6,7), FUN='+')  #row + x# margin keep is as each row and stats means addd by 2, 3, 4, 5,6, 7
...
M3functions = addmargins(m3, margin=c(1,2), list(list(mean, sum, max, min), list(var, sd, max, min)))# i am giving on column side maximum and minimum sale. and on row I am trying to get company wise data as a row , mean sum , maximum and minimum
M3functions
round(M3functions,1)
...
(a1 = array(100:123, dim=c(4,3,2)))# 2 matrice, four row & 3 columns)
